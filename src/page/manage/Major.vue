<template>
    ...
    <el-drawer v-model="drawer.show" :before-close="cancelForm" title="编辑专业信息" direction="rtl" size="500px">
        <div class="edit-drawer__content">
            <el-form ref="baseForm" :model="formData" label-width="auto" :rules="rules">
                ...
                <el-form-item label="二级学院" prop="college_id">
                    <el-select-v2 v-model="formData.college_id" filterable :options="collegeData.value" remote
                        :remote-method="fetchSelectData" :loading="collegeData.loading" placeholder="请选择">
                        <template #loading>
                            <el-icon>
                                <Loading />
                            </el-icon>
    ...
    setup() {
        ...
        const handleDelete = (index, row) => {
            ElMessageBox.confirm("是否确认删除？删除该数据可能会导致其他关联数据错误。", "提示", {
                ...
                .then(() => {
                    ...
                    window.$axios.post("/entity/major/delete", {
                        "major_id": row.id
                    }, {
                        headers: {
        ...
        const updateData = () => {
            ...
            baseForm.value.validate((valid) => {
                ...
                if (formData.id == 'auto') {
                    // 新增
                    window.$axios.post("/entity/major/add", {
                        "name": formData.name,
                    ...
                    window.$axios.post("/entity/major/update", {
                        "major_id": formData.id,
                        "name": formData.name,
                        "college_id": formData.college_id,
                    }, {
                        headers: {
            ...
            window.$axios.get("/entity/college/all", {
                ...
                if (response.status == 200) {
                    if (response.data.code == 200) {
                        ...
                        collegeData.value = opt.filter((item) => {
